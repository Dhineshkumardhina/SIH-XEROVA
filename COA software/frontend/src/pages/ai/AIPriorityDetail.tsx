...
export const AIPriorityDetail: React.FC = () => {
  ...
  if (error || !prediction) {
    return <div className="p-8 text-center text-red-400">{error || 'Not found'}</div>
  }

  const factorEntries = Object.entries(prediction.factor_breakdown)
    .sort(([, a], [, b]) => b.contribution - a.contribution)

  const highContrib = factorEntries.filter(([, f]) => f.contribution >= 15)
  const medContrib = factorEntries.filter(([, f]) => f.contribution >= 5 && f.contribution < 15)
  const lowContrib = factorEntries.filter(([, f]) => f.contribution < 5)

  return (
    <div className="space-y-6 pb-20">
      <PageHeader
        title={`AI Priority Analysis`}
        subtitle={`Detailed explainability for maintenance task ${prediction.task_id}`}
        breadcrumbs={[
          { label: 'AI Priority', href: '/ai/priority' },
          { label: prediction.task_id }
        ]}
        actions={
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="p-8 text-center">
            ...
            <div className="text-6xl font-black font-mono tracking-tight text-white mb-4">
              {prediction.priority_score.toFixed(1)} <span className="text-2xl text-slate-500">/ 100</span>
            </div>
                ...
                {factorEntries.map(([key, factor]) => (
                  <div key={key} className="bg-slate-800/40 rounded-lg p-4 border border-slate-700/50">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="font-mono font-bold text-indigo-300">+{factor.contribution.toFixed(2)}</span>
                    </div>
                    <div className="w-full bg-slate-900 rounded-full h-1.5 mb-2 overflow-hidden">
                      <div className="bg-indigo-500 h-1.5" style={{ width: `${Math.min((factor.contribution / 25) * 100, 100)}%` }}></div>
                    </div>
                    <div className="flex justify-between text-xs text-slate-400 font-mono">
                      <span>Raw: {factor.raw_value}</span>
              ...
              <div className="space-y-4">
                {highContrib.length > 0 && (
                  <div>
                    ...
                    <ul className="text-sm text-slate-300 space-y-1 pl-4 list-disc">
                      {highContrib.map(([k, f]) => <li key={k} className="capitalize">{k.replace(/_/g, ' ')} (+{f.contribution.toFixed(2)})</li>)}
                    </ul>
                  </div>
                )}
                ...
                {medContrib.length > 0 && (
                  <div>
                    ...
                    <ul className="text-sm text-slate-300 space-y-1 pl-4 list-disc">
                      {medContrib.map(([k, f]) => <li key={k} className="capitalize">{k.replace(/_/g, ' ')} (+{f.contribution.toFixed(2)})</li>)}
                    </ul>
                  </div>
                )}
                ...
                {lowContrib.length > 0 && (
                  <div>
                    ...
                    <ul className="text-sm text-slate-300 space-y-1 pl-4 list-disc">
                      {lowContrib.map(([k, f]) => <li key={k} className="capitalize">{k.replace(/_/g, ' ')} (+{f.contribution.toFixed(2)})</li>)}
                    </ul>
                  </div>
                )}
              </div>
          ...
          <Card>
            ...
            <div className="p-6 text-sm space-y-3 text-slate-400">
              <div className="flex justify-between">
                <span>Model:</span>
                <span className="text-slate-200">{prediction.model_name} v{prediction.model_version}</span>
              </div>
              <div className="flex justify-between">
                <span>Training Data:</span>
                <span className="text-slate-200">None (Rule-based heuristics)</span>
              </div>
              <div className="flex justify-between">
